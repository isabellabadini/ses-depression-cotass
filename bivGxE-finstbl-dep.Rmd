---
title: "Bivariate GxE: Depression moderated by financial stability  (CoTASS)"
author: "IB"
date: "19/07/2021"
output: 
  html_document:
...
data$FSm1 <- data$finstbl_rS1
data$FSm2 <- data$finstbl_rS2
describe(data$FSm1)
describe(data$FSm2)
...
Vars <- c('finstbl_rS','depTS')
selVars <- paste(Vars,c(rep(1,nv),rep(2,nv)),sep="")
selVarsS <- c('finstbl_rS1','depTS1')
defVars <- c('FSm1','FSm2')

# Drop cases with missingness on the definition variables

#mzData <-data[c(data$c1c2_sexzyg==1 | data$c1c2_sexzyg==3) & !is.na(data$FSm1) & !is.na(data$FSm2), c(selVars, c('FSm1','FSm2'))]
#dzData <-data[c(data$c1c2_sexzyg==2 | data$c1c2_sexzyg==4 | data$c1c2_sexzyg == 5) & !is.na(data$FSm1) & !is.na(data$FSm2), c(selVars, c('FSm1','FSm2'))]
#sData <-data[data$c1c2_sexzyg==6 & !is.na(data$finstbl_rS1), c(selVarsS, 'FSm1')]

mzmData	<-data[data$c1c2_sexzyg==1 & !is.na(data$finstbl_rS1) & !is.na(data$finstbl_rS2), c(selVars, c('FSm1','FSm2'))]
dzmData	<-data[data$c1c2_sexzyg==2 & !is.na(data$finstbl_rS1) & !is.na(data$finstbl_rS2), c(selVars, c('FSm1','FSm2'))]
mzfData	<-data[data$c1c2_sexzyg==3 & !is.na(data$finstbl_rS1) & !is.na(data$finstbl_rS2), c(selVars, c('FSm1','FSm2'))]
dzfData	<-data[data$c1c2_sexzyg==4 & !is.na(data$finstbl_rS1) & !is.na(data$finstbl_rS2), c(selVars, c('FSm1','FSm2'))]
dzoData	<-data[data$c1c2_sexzyg==5 & !is.na(data$finstbl_rS1) & !is.na(data$finstbl_rS2), c(selVars, c('FSm1','FSm2'))]
smData	<-data[data$c1c2_sexzyg==6 & data$sex1==0 & !is.na(data$finstbl_rS1), c(selVarsS, 'FSm1')] # males
sfData	<-data[data$c1c2_sexzyg==6 & data$sex1==1 & !is.na(data$finstbl_rS1), c(selVarsS, 'FSm1')] # females
...
defFSm1   <- mxMatrix(type="Full", nrow=1, ncol=1, free=FALSE, labels=c("data.FSm1"), name="FSm1")
defFSm2   <- mxMatrix(type="Full", nrow=1, ncol=1, free=FALSE, labels=c("data.FSm2"), name="FSm2")
...
varAm1    <- mxAlgebra(expression=(a+ FSm1%x%aMod) %*% t(a+ FSm1%x%aMod), name="Am1")
varCm1    <- mxAlgebra(expression=(c+ FSm1%x%cMod) %*% t(c+ FSm1%x%cMod), name="Cm1")
varEm1    <- mxAlgebra(expression=(e+ FSm1%x%eMod) %*% t(e+ FSm1%x%eMod), name="Em1")

varAm2    <- mxAlgebra(expression=(a+ FSm2%x%aMod) %*% t(a+ FSm2%x%aMod), name="Am2")
varCm2    <- mxAlgebra(expression=(c+ FSm2%x%cMod) %*% t(c+ FSm2%x%cMod), name="Cm2")
varEm2    <- mxAlgebra(expression=(e+ FSm2%x%eMod) %*% t(e+ FSm2%x%eMod), name="Em2")

covAm12   <- mxAlgebra(expression=(a+ FSm1%x%aMod) %*% t(a+ FSm2%x%aMod), name="Am12")
covCm12   <- mxAlgebra(expression=(c+ FSm1%x%cMod) %*% t(c+ FSm2%x%cMod), name="Cm12")
covAm21   <- mxAlgebra(expression=(a+ FSm2%x%aMod) %*% t(a+ FSm1%x%aMod), name="Am21")
covCm21   <- mxAlgebra(expression=(c+ FSm2%x%cMod) %*% t(c+ FSm1%x%cMod), name="Cm21")
...
#covS		<-mxAlgebra(((a+aMod*FSm1)*t(a+aMod*FSm1)) + ((c+cMod*FSm1)*t(c+cMod*FSm1)) + ((e+eMod*FSm1)*t(e+eMod*FSm1)), name="Vm1") # singletons 
...
ScalarOS <-mxMatrix(type="Diag", nrow=4, ncol=4, free=c(F,T,F,T), values=c(1,.6,1,.6), label=c("NA","dep1", "NA", "dep1"), name="ScOS")	# M-F pairs
...
covMs	<- mxAlgebra(((a+aMod*FSm1)*t(a+aMod*FSm1)) + ((c+cMod*FSm1)*t(c+cMod*FSm1)) + ((e+eMod*FSm1)*t(e+eMod*FSm1)), name="Vm1")
covFs	<- mxAlgebra(expression=Scs%&%((a+aMod*FSm1)*t(a+aMod*FSm1)) + ((c+cMod*FSm1)*t(c+cMod*FSm1)) + ((e+eMod*FSm1)*t(e+eMod*FSm1)), name="Vm1")
...
#defs    <- list(defFSm1, defFSm2, varAm1, varCm1, varEm1, covPm1, varAm2, varCm2, varEm2, covPm2, covAm12, covCm12, covAm21, covCm21)
#modelMZ <- mxModel(pars, defs, covMZm, dataMZ, expMZ, funML, name="MZ")
#modelDZ <- mxModel(pars, defs, covDZm, dataDZ, expDZ, funML, name="DZ")
#modelSs	<- mxModel(pars, defFSm1, varAm1, varCm1, varEm1, covPm1, covS, dataS, expS, funML,name="Ss") # singletons   
...
modelSM  <- mxModel(MeanMs, pars, defFSm1, varAm1, varCm1, varEm1, covPm1, covMs, dataSM, objSM, funML, name="SM")
modelSF  <- mxModel(MeanFs, ScalarFs, pars, defFSm1, varAm1, varCm1, varEm1, covPm1, covFs, dataSF, objSF, funML, name="SF")
...
write.csv(GxEpar, "bivGxE-FS-dep-par.csv",quote=T)
write.csv(GxECID, "bivGxE-FS-dep-cid.csv",quote=T)
write.csv(GxEVars, "bivGxE-FS-dep-vars.csv",quote=T)
...
write.csv(Nomodpar, "bivnomod-FS-dep-par.csv",quote=T)
write.csv(NomodCID, "bivnomod-FS-dep-cid.csv",quote=T)
write.csv(NomodVars, "bivnomod-FS-dep-vars.csv",quote=T)
write.csv(mxcomp, "bivmxcomp-FS-dep-vars.csv",quote=T)
...
write.csv(smod2par, "bivsmod2-FS-dep-par.csv",quote=T)
write.csv(smod2CID, "bivsmod2-FS-dep-cid.csv",quote=T)
write.csv(mxcomp2, "bivmxcomp2-FS-dep-vars.csv",quote=T)

write.csv(smod3par, "bivsmod3-FS-dep-par.csv",quote=T)
write.csv(smod3CID, "bivsmod3-FS-dep-cid.csv",quote=T)
write.csv(mxcomp3, "bivmxcomp3-FS-dep-vars.csv",quote=T)

write.csv(Nested.fit,"bivmod-FS-nest-fit.csv",quote=T)
...
modVal <- sort(unique(c(data$FSm1,data$FSm2)))
...
matplot(
  ...
  xlab="Moderator: Financial Stability",
